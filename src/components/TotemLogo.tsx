...
export function TotemLogo({ className = "size-12", loading = false }: Props) {
  const wrapperClasses = ["totem-logo", loading && "totem-logo--loading", className]
    .filter(Boolean)
    .join(" ");

  return (
    <span className={wrapperClasses} aria-hidden>
