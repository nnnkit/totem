...
12. Update `docs/blog-pipeline.md` with status, URL, canonical keyword, refresh
...
while goal is not complete:
  1. Refresh context:
     ...
     - List current posts in apps/site/src/content/blog/ and compare them with
       the Published table in docs/blog-pipeline.md. If the table is stale,
  ...
  4. Choose the next item:
     ...
     - Prefer a tool proposal when users want an action or workflow, not another
       explanation.
     - If a topic fails the fit check, move it to Killed in docs/blog-pipeline.md
  ...
  7. Draft, refresh, or propose:
     ...
     - For tool ideas, write the proposal into the HTML report and add the item
       to docs/blog-pipeline.md or the appropriate product plan.
...
```txt
Use docs/seo-agent-goal.md as your operating instructions. Keep running the
...
update docs/blog-pipeline.md, record a readable HTML research/status report
...
`docs/blog-pipeline.md`. If extension screenshots are involved, read
...
- `docs/blog-pipeline.md` is updated.
